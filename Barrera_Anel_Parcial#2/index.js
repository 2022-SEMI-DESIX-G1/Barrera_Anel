((Utils) => {
    const App = {
      ...
      handlers: {
        pokemonFinderFormOnSubmit: async (e) => {
          ...
          try {
           ...
           // const species = response.data.species.json();
             
            const renderedTemplate = App.templates.render({
              searchType,
              response,
            });
            App.htmlElements.pokemonClean.style.visibility = "visible";
        ...
        pokemonFinderFormOnClean: async (e) => {
          e.preventDefault();
          App.htmlElements.pokemonFinderOutput.style.visibility = "hidden";
          App.htmlElements.pokemonClean.style.visibility = "hidden";
      ...
      templates: {
        ...
        pokemonCard: ({ id, name, weight, height,evolutionChain,sprites,abilities }) => {  
          ...
          //return `<h1>${name} (${id})</h1><ul><li>Weight: ${weight}</li><li>Height: ${height}</li><ul>${evoChainList.join("")}</ul></ul>`;
        ...
        abilityCard: ({ id, name, pokemon }) => {
          const pokemonList = pokemon.map(
            ({ pokemon, is_hidden }) =>
              `<li>${pokemon.name}${
                is_hidden ? `<svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
