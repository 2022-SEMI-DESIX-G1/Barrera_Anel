...
exports.getLocation = async (req, res) => {
    ...
    if (CACHE[location]) {
        return res.json({ name, data: JSON.parse(CACHE[location]), isCached: true });
      }
      if (ERROR[location]) {
        return res.json({ name, data: JSON.parse(ERROR[location]), isCached: true });
      }

    const api = await fetch(url); if (api.ok) {
    const data = await api.json(); console.log(data); try {
    //res.json(data);
    CACHE[location] = JSON.stringify(data);
    res.json({ name, data: data, isCached: false });
    } catch (error) {
    console.log(error);
    ERROR[location] = JSON.stringify({ name, error: "Invalid Location." });
    }
    }
};